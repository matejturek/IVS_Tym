...
{
    math_lib tmp;
    QVERIFY(tmp.addition(0, 0) == 0);
}

void math_lib_test::addition3()
{
    math_lib tmp;
    QCOMPARE(tmp.addition(-99999, 2), -99997);
}

void math_lib_test::addition4()
{
    math_lib tmp;
    QCOMPARE(tmp.addition(-7, -42), -49);
}

void math_lib_test::addition5()
{
    math_lib tmp;
    QCOMPARE(tmp.addition(66, 100), 166);
}

void math_lib_test::subtraction1()
{
    math_lib tmp;
    QCOMPARE(tmp.subtraction(5, 4), 1);
}

void math_lib_test::subtraction2()
{
    math_lib tmp;
    QCOMPARE(tmp.subtraction(0, 0), 0);
}

void math_lib_test::subtraction3()
{
    math_lib tmp;
    QCOMPARE(tmp.subtraction(-99999, 2), -100001);
}

void math_lib_test::subtraction4()
{
    math_lib tmp;
    QCOMPARE(tmp.subtraction(-7, -42), 35);
}

void math_lib_test::subtraction5()
{
    math_lib tmp;
    QCOMPARE(tmp.subtraction(66, 100), -34);
}
...
{
    math_lib tmp;
    QCOMPARE(tmp.multiplication(5, 4), 20);
}

void math_lib_test::multiplication2()
{
    math_lib tmp;
    QCOMPARE(tmp.multiplication(42, 0), 0);
}

void math_lib_test::multiplication3()
{
    math_lib tmp;
    QCOMPARE(tmp.multiplication(-99999, 2), -199998);
}

void math_lib_test::multiplication4()
{
    math_lib tmp;
    QCOMPARE(tmp.multiplication(-7, -42), 294);
}

void math_lib_test::multiplication5()
{
    math_lib tmp;
    QCOMPARE(tmp.multiplication(66, 100), 6600);
}

void math_lib_test::factorial1()
{
    math_lib tmp;
    QCOMPARE(tmp.factorial(-5), 0);
}

void math_lib_test::factorial2()
{
    math_lib tmp;
    QCOMPARE(tmp.factorial(1), 1);
}

void math_lib_test::factorial3()
{
    math_lib tmp;
    QCOMPARE(tmp.factorial(3), 6);
}

void math_lib_test::factorial4()
{
    math_lib tmp;
    QCOMPARE(tmp.factorial(10), 3628800);
}

void math_lib_test::factorial5()
{
    math_lib tmp;
    QCOMPARE(tmp.factorial(5), 120);
}

void math_lib_test::power1()
{
    math_lib tmp;
    QCOMPARE(tmp.power(-5, 0), 1);
}

void math_lib_test::power2()
{
    math_lib tmp;
    QCOMPARE(tmp.power(1, 1), 1);
}

void math_lib_test::power3()
{
    math_lib tmp;
    QCOMPARE(tmp.power(3, -5), 0);
}

void math_lib_test::power4()
{
    math_lib tmp;
    QCOMPARE(tmp.power(10, 10), 10000000000);
}

void math_lib_test::power5()
{
    math_lib tmp;
    QCOMPARE(tmp.power(5,2), 25);
}

void math_lib_test::root1()
{
    math_lib tmp;
    QCOMPARE(tmp.root(-5, -2), 0);
}

void math_lib_test::root2()
{
    math_lib tmp;
    QCOMPARE(tmp.root(1, 1), 1);
}
